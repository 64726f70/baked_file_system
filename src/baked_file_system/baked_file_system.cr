...
module BakedFileSystem
  ...
  class BakedFile < IO
    ...
    def to_slice(compressed)
      return to_slice if compressed

      Bytes.new(size).tap { |slice| read slice }
    end
  end
  ...
  macro bake_folder(path, dir = __DIR__, allow_empty = false)
    ...
    {% unless allow_empty %}
      raise "BakedFileSystem empty: no files in #{File.expand_path({{ path }}, {{ dir }})}" if (@@files.size - %files_size_ante).zero?
    {% end %}
  end
